...
function Logs() {
	...
	const base200 = getTailwindColorHex('text-base-200');
	const base300 = getTailwindColorHex('text-base-300');
	const primary = getTailwindColorHex('text-primary');
	const secondary = getTailwindColorHex('text-secondary');
	const neutral = getTailwindColorHex('text-neutral');
	const neutralContent = getTailwindColorHex('text-neutral-content');
	...
	for (let i = 0; i < rowsData.length; i++) {
		const { statuscode, method, api, details, userId, ipaddress, time, country, city } =
			rowsData[i];
	...
	return (
		<>
			...
			>
				<h1 className="mt-20 text-3xl font-bold leading-tight mb-6">Logs</h1>
				<Paper
					sx={{
						width: '100%',
						overflow: 'hidden',
				...
				>
					<div className="mb-4 border-b border-gray-200 dark:border-gray-700">
						<ul className="flex text-sm font-medium text-center" id="myTab" role="tablist">
							<li className="flex-grow mr-2" role="presentation">
								<button
									className={`w-full inline-block p-4 border-b-2 rounded-t-lg text-neutral-content hover:text-secondary ${
										activeTab === 'basic-logs'
									...
									{columns.map(
										(column) =>
											(activeTab === 'advanced-logs' ||
												...
												>
													<div className="bg-base-200 font-bold p-4">
														{column.label}
													</div>
												</TableCell>
											)
					...
					<div className="flex justify-between items-center">
						<button
							className="bg-base-100 px-4 py-2 rounded focus:outline-none"
							type="button"
							onClick={(e) => replay(e)}
						>
							<ReplayIcon />
