...
function UserCard({ userType, userName, email, userID, profilePicture }) {
	...
	return (
		<div className="w-full max-w-sm bg-base-200 border border-base-300 rounded-lg shadow hover:border-secondary">
			...
			<div className="flex flex-col items-center pb-10">
				{renderImage()}
				<h5 className="mb-1 text-xl font-medium">{userName}</h5>
				<span className="text-sm">{email}</span>
				<span className="text-sm">User ID: {userID}</span>
				<div className="flex mt-4 md:mt-6">
					<Link to={'/profile/' + userID} className="btn btn-outline mr-4">
						View Profile
					</Link>

					<button
						type="button"
						className="btn btn-primary inline-flex items-center px-4 py-2 text-sm font-medium text-center rounded-lg"
					>
						Change Role
					</button>
				</div>
